...
export async function fetchJson(url, { headers = {}, label = url, attempts } = {}) {
  ...
  try {
    ...
    const endpoint = response.url || String(url);
    const prefix = JSON.stringify(body.slice(0, 160));
    throw new Error(
      `${label} returned invalid JSON from ${endpoint}: ${response.status} ${response.statusText}; content-type: ${contentType}; body prefix: ${prefix}`,
      { cause }
    );
  }
}
