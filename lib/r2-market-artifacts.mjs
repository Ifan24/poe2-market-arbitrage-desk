...
import { getConfiguredActiveLeagueShortNames, getCurrentSoftcoreLeagues } from "./poe2scout-leagues.mjs";
...
export function selectPublishedSoftcoreLeagues(leagues, { league, env = process.env } = {}) {
  const current = getCurrentSoftcoreLeagues(leagues);
  const configured = getConfiguredActiveLeagueShortNames(env);
  if (!configured) return current;
  const defaultLeagueId = (env.POE2SCOUT_DEFAULT_LEAGUE_SHORT_NAME || "forbiddenrites").trim().toLowerCase();
  for (const required of [league.ShortName, defaultLeagueId]) {
    if (!configured.includes(required)) {
      throw new Error(`POE2SCOUT_ACTIVE_LEAGUES_JSON must include league ${required}`);
    }
  }
  return configured.map((id) => {
    const candidate = current.find((entry) => entry.ShortName === id);
    if (!candidate) {
      throw new Error(`configured poe2scout league ${id} is not a current softcore league`);
    }
    return candidate;
  });
}
...
export function buildMarketArtifactBundle({
  ...
  league,
  leagues = [],
  defaultLeagueId = process.env.POE2SCOUT_DEFAULT_LEAGUE_SHORT_NAME || "forbiddenrites",
  provider = "poe2scout",
  realm = "poe2",
  now = new Date()
}) {
  defaultLeagueId = defaultLeagueId.trim().toLowerCase();
  const leagueId = league.ShortName;
  const leagueName = league.Value;
  if (snapshot?.league !== leagueName || (snapshot.leagueId && snapshot.leagueId !== leagueId)) {
    throw new Error(`snapshot league does not match selected league ${leagueId}`);
  }
  ...
  const manifest = {
    ...
    realm,
    defaultLeagueId,
    availableLeagues: getCurrentSoftcoreLeagues(leagues).map((candidate) => ({
      id: candidate.ShortName,
      name: candidate.Value,
      hardcore: false,
      manifest: { url: `/leagues/${candidate.ShortName}/manifest.json` }
    })),
    status: { url: `/leagues/${leagueId}/status.json` },
    trendIndex: { url: `/leagues/${leagueId}/trend-index.json` },
    activeLeague: {
      id: leagueId,
      name: leagueName,
      hardcore: false
  ...
  return {
    publishRoot: leagueId === defaultLeagueId,
    manifest,
    status,
    snapshotKey,
    keys: {
...
export async function uploadMarketArtifacts(bundle, { env = process.env, client = createR2Client(env) } = {}) {
  ...
  ].filter((upload) => bundle.publishRoot || upload.key.startsWith(`leagues/${bundle.manifest.activeLeague.id}/`));
...
export function buildRefreshSummary({ bundle, uploads, env = process.env, itemCount, icons }) {
  ...
  const manifestKey = bundle.publishRoot ? bundle.keys.rootManifest : bundle.keys.leagueManifest;
  const statusKey = bundle.publishRoot ? bundle.keys.rootStatus : bundle.keys.leagueStatus;
  const manifestUrl = baseUrl ? `${baseUrl}/${manifestKey}` : manifestKey;
  const statusUrl = baseUrl ? `${baseUrl}/${statusKey}` : statusKey;
